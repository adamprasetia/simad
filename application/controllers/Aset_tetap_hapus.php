...
class Aset_tetap_hapus extends MY_Controller {

	private $limit = 15;
	private $table = 'aset_tetap_hapus';
	...
	public function index()
	{
		...
		$this->_filter();
		$aset_tetap_hapus_view['data'] 	= $this->db->order_by('id desc')->get($this->table, $this->limit, $offset)->result();
		$aset_tetap_hapus_view['offset'] = $offset;
		$aset_tetap_hapus_view['paging'] = gen_paging($total,$this->limit);
		$aset_tetap_hapus_view['total'] 	= gen_total($total,$this->limit,$offset);
		$data['content'] 	= $this->load->view('contents/aset_tetap_hapus_view', $aset_tetap_hapus_view, TRUE);
	...
	public function add()
	{
		...
		if ($this->form_validation->run()===FALSE) {
			$data['content'] = $this->load->view('contents/form_aset_tetap_hapus_view', [
				'action'=>base_url('aset_tetap_hapus/add').get_query_string(),
			],true);
			...
			if(empty($error['message'])){
				$response = array('id'=>$id, 'redirect'=>base_url('aset_tetap_hapus_detail/add?id_aset_tetap_hapus='.$id), 'action'=>'insert', 'message'=>'Data berhasil disimpan');
			}else{
	...
	public function edit($id='')
	{
		...
		if ($this->form_validation->run()===FALSE) {
			$this->db->where('id', $id);
			$aset_tetap_hapus_view['data'] = $this->db->get($this->table)->row();
			$aset_tetap_hapus_view['action'] = base_url('aset_tetap_hapus/edit/'.$id).get_query_string();
			$data['content'] = $this->load->view('contents/form_aset_tetap_hapus_view',$aset_tetap_hapus_view,true);
	...
	public function delete($id = '')
	{
		if ($id) {
			$this->db->trans_start();
			$detail = $this->db->where('id_aset_tetap_hapus', $id)->get('aset_tetap_hapus_detail')->result();
